...
function analyzeMatch(match) {
  ...
  // Calculate all goal markets
  const over15Prob = Math.min(95, Math.round(50 + (totalXg * 15)));
  const over25Prob = Math.min(88, Math.max(25, Math.round(30 + (totalXg * 14))));
  const over35Prob = Math.min(75, Math.max(15, Math.round(15 + (totalXg * 12))));
  const over45Prob = Math.min(55, Math.max(5, Math.round(5 + (totalXg * 10))));

  const bttsProb = Math.min(85, Math.max(30, Math.round(30 + (Math.min(homeXg, awayXg) * 25))));
  ...
  else matchResultPrediction = "DRAW";

  // Double Chance Prediction
  let doubleChancePrediction = "";
  if ((homeWinProb + drawProb) > (awayWinProb + drawProb) && (homeWinProb + drawProb) > (homeWinProb + awayWinProb)) {
    doubleChancePrediction = "HOME OR DRAW (1X)";
  } else if ((awayWinProb + drawProb) > (homeWinProb + drawProb)) {
    doubleChancePrediction = "AWAY OR DRAW (X2)";
  } else {
    doubleChancePrediction = "HOME OR AWAY (12)";
  }
  ...
  const candidates = [
    ...
    { name: "HOME DOUBLE CHANCE (1X)", prob: homeWinProb + drawProb },
    { name: "AWAY DOUBLE CHANCE (X2)", prob: awayWinProb + drawProb },
    { name: "OVER 1.5 GOALS", prob: over15Prob },
    { name: "OVER 2.5 GOALS", prob: over25Prob },
    { name: "BTTS - YES", prob: bttsProb }
  ];
  candidates.forEach(function(c) {
    if (c.prob > safestProb) { safestProb = c.prob; safestBet = c.name; }
  });
...
app.get('/api/today', async (req, res) => {
  ...
  try {
    ...
    const listResponse = await axios.get(listUrl, { headers: { Authorization: "Token " + API_KEY }, timeout: 8000 });
